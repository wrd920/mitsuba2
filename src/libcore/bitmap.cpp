...
void Bitmap::rebuild_struct(size_t channel_count, const std::vector<std::string> &channel_names) {
    ...
    for (auto ch: channels) {
        ...
        if ((ch != "A" || m_pixel_format == PixelFormat::MultiChannel) && ch != "W" && m_srgb_gamma)
            flags |= +Struct::Flags::Gamma;
        if ((ch != "A" || m_pixel_format == PixelFormat::MultiChannel) && ch != "W" && m_premultiplied_alpha)
            flags |= +Struct::Flags::PremultipliedAlpha;
        if (ch == "A" && m_pixel_format != PixelFormat::MultiChannel)
...
std::vector<std::pair<std::string, ref<Bitmap>>> Bitmap::split() const {
    ...
    for (auto it = fields.begin(); it != fields.end();) {
        ...
        ref<Struct> target_struct = new Struct(*target->struct_());
        auto link_field = [&](const std::string l, FieldMap::iterator it) {
            target_struct->field(l).name = it->second.second->name;
            fields.erase(it);
        };

        for (auto it2 = range.first; it2 != range.second; ++it2) {
            link_field(it2->second.first, it2);
        }
